...
def load_llm():
    """
    FIX: main.py imported load_llm from this module but it was never defined,
    causing an ImportError at startup. The LLM is accessed via get_groq_client()
    at call time (no persistent model object to load), so this function is a
    no-op stub that satisfies the import without breaking anything.
    Returns None — generate_report() calls get_groq_client() internally.
    """
    return None
...
def build_prompt(input_context, rag_context: str, anomalies: list) -> str:
    ic = input_context
...
def generate_report(input_context, rag_context: str = "", anomalies: list = None) -> str:
    ...
    FIX: previously the return value was stored in ic.final_report (here) but
    app.py read ic.report — so the report tab always showed "Report not generated."
    Now generate_report() returns the string and app.py stores it in ic.report
    directly (see app.py step 6). The function signature also accepts keyword
    args with defaults so it works whether called as generate_report(ic, ctx, anoms)
    or generate_report(ic, rag_context=ctx, anomalies=anoms).
    """
